...
from app.models.position import PointArret
...
def add_position():
    ...
    if last_position:
        ...
        if d < 5:  # ignore les sauts GPS aberrants (perte de signal, etc.)
...
def historique():
    user_id = get_jwt_identity()
    periode = request.args.get('periode', 'mois')  # jour, semaine, mois, tout
